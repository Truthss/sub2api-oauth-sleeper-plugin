...
# Avoid importing package dependencies (asyncpg/pydantic) for pure logic tests.
...
def test_respects_min_remaining_and_max_per_scan() -> None:
    now = datetime(2026, 5, 16, tzinfo=timezone.utc)
    accounts = [account(1, 100), account(2, 99), account(3, 98), account(4, 10), account(5, 5)]
    selected = select_sleep_candidates(accounts, 90, now, "openai", 3, 4)
    assert_equal([a["id"] for a, _ in selected], [1], "min remaining selection")
    assert_equal(selected[0][1].window_name, "7d", "decision window")


def test_blocks_when_floor_would_be_breached() -> None:
    now = datetime(2026, 5, 16, tzinfo=timezone.utc)
    selected = select_sleep_candidates([account(1, 100), account(2, 99), account(3, 10)], 90, now, "openai", 10, 3)
    assert_equal(selected, [], "floor breach selection")
...
def test_orders_by_highest_utilization_first() -> None:
    ...
    selected = select_sleep_candidates(accounts, 90, now, "openai", 2, 0)
    assert_equal([a["id"] for a, _ in selected], [2, 3], "utilization ordering")


if __name__ == "__main__":
    tests = [
        test_respects_min_remaining_and_max_per_scan,
        test_blocks_when_floor_would_be_breached,
